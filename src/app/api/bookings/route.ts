...
const createBookingReference = () => `LH-${Date.now()}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!authorization.startsWith("Bearer ")) {
      return NextResponse.json({ error: "Guest authentication is required." }, { status: 401 });
    }
    try {
      await getLighthouseAdminAuth().verifyIdToken(authorization.slice(7));
    } catch {
      return NextResponse.json({ error: "Guest authentication is invalid or expired." }, { status: 401 });
    }
    ...
    let backendSyncStatus: WebsiteBookingBackendSyncStatus = backendUrl ? "pending" : "synced";
