...
export default function OverviewPage() {
  ...
  useEffect(() => {
    ...
    const refreshOverview = () => {
      ...
      const cashierSnapshot = readCashierState<CashierTransaction>("lighthouse-cashier-transactions", "lighthouse-cashier-seq", 1);
      const kitchenSnapshot = readPosState<QueueTicket, POSPaymentRecord, unknown>(activeKitchenKey, "lighthouse-kitchen-tickets", "lighthouse-kitchen-seq", "lighthouse-kitchen-payments", "lighthouse-kitchen-menu", 1);
      const baristaSnapshot = readPosState<QueueTicket, POSPaymentRecord, unknown>(activeBaristaKey, "lighthouse-barista-orders", "lighthouse-barista-seq", "lighthouse-barista-payments", "lighthouse-barista-menu", 1);
    ...
    void Promise.all([
      ...
      hydrateStorageKeyFromFirebase("lighthouse-rooms-state", true),
    ]).finally(refreshOverview);

    const unsubscribeCashier = subscribeToSyncedStorageKey("lighthouse-cashier-state", refreshOverview);
    ...
    const unsubscribeRooms = subscribeToSyncedStorageKey("lighthouse-rooms-state", refreshOverview);
  ...
  const netRevenueChartData = useMemo(
    () => [
      { name: "Booking", value: bookingRevenue, fill: "#B98025" },
  ...
  const stats = useMemo(
    () => [
      ...
      { label: "Kitchen Queue", value: `${activeKitchen}`, icon: TrendingUp, trend: activeKitchen > 5 ? "High" : "Stable", trendUp: activeKitchen <= 5, color: "text-amber-500" },
