...
const STAFF_STORAGE_KEY = "lighthouse-staff-members";
...
export default function StaffPage() {
  ...
  const saveRoleProfile = async (role: Role, username: string, blocked = false) => {
    const profiles = readLocalLoginProfiles() ?? {};
    const nextEntry = upsertProfileUser(profiles[role], username, {
      password: getDefaultLoginPassword(role),
      blocked,
      updatedAt: Date.now(),
    });
  ...
  const addMember = async () => {
    ...
    await saveRoleProfile(newRole, username, false);
    setNewName("");
    setNewRole("cashier");
    setNewShift("Day");
  ...
  const updateMemberRole = async (memberId: string, role: Role) => {
    ...
    await saveRoleProfile(role, currentMember.name, currentMember.blocked === true);
  };
  ...
  const toggleMemberBlocked = async (memberId: string) => {
    ...
    await saveRoleProfile(currentMember.role, currentMember.name, nextBlocked);
  };
