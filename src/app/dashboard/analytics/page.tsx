...
const COLORS = ["#B98025", "#111111", "#00A676", "#2F80ED", "#2563EB", "#D92D20", "#8E44AD", "#64748B"];
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(() => hydrateStorageKeyFromFirebase("lighthouse-kitchen-payments", true))
      .finally(applyAnalyticsSnapshot);

    const unsubscribers = [
      subscribeToSyncedStorageKey("lighthouse-cashier-state", applyAnalyticsSnapshot),
      subscribeToSyncedStorageKey(activeKitchenKey, applyAnalyticsSnapshot),
      subscribeToSyncedStorageKey("lighthouse-kitchen-payments", applyAnalyticsSnapshot),
  ...
  const aspectRows = useMemo(
    () => [
      { label: "Rooms", revenue: totals.roomRevenue, expenses: periodExpenses.filter((expense) => expense.department === "rooms").reduce((sum, expense) => sum + expense.amount, 0), color: "#B98025" },
  ...
  const exportFinancialStatement = () => {
    const rows: Array<[string, string | number]> = [
      ["Lighthouse MOTEL FINANCIAL STATEMENT", activeReportLabel],
    ...
    link.download = `lighthouse-financial-statement-${periodSlug || range}.csv`;
    link.click();
        ...
        <div className="space-y-4">
          ...
          <Card className="overflow-hidden border-none bg-white shadow-lg">
            <div className="bg-black px-5 py-6 text-white md:px-8">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Lighthouse Motel</p>
        ...
        <Card className="rounded-lg border-none bg-white shadow-sm">
          ...
          <CardContent className="space-y-4">
            {[
              ...
              { label: "Credit Exposure", value: creditExposure, tone: "text-amber-600" },
            ].map((item) => (
                ...
                <AreaChart data={history}>
                  <defs>
                    <linearGradient id="colorRev" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#B98025" stopOpacity={0.1} />
                      <stop offset="95%" stopColor="#B98025" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                  ...
                  />
                  <Legend wrapperStyle={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" }} />
                  <Area type="monotone" dataKey="totalRevenue" name="Gross Revenue" stroke="#B98025" strokeWidth={4} fillOpacity={1} fill="url(#colorRev)" />
